...
function drawBookList(res) {
    ...
    res.forEach(function (book) {
        ...
        button.addEventListener('click', deleteFromList(book.id_book, tr));
        button.textContent = "Remove";
        a.setAttribute('href', `/book/${book.id_book}`)
        a.text = book.book_title;
        td1.appendChild(a);
...
function getChapters(page) {
    return fetch(`/books/my/reading/back?page=${page}`)
...
function deleteFromList(id, trToHide) {
    return async () => {
        fetch(`/books/my/reading/delete/${id}`, {method: "DELETE"})
            .then(function () {
                message.style.display = 'block';
